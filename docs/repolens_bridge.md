...
> Maps RepoLens runtime agents to Repo-Analyser PML/OCL eval tasks. Use RepoLens for **static repo analysis**; use Repo-Analyser agent specs for **skill eval execution** (build, docker, CI, worktrees).

## When to Use Which

| Goal | Use |
|------|-----|
| Scan unfamiliar Java/Python/Node repo | RepoLens `RepoDiscoveryAgent` → Repo-Analyser **B1** |
| Map HTTP endpoints | RepoLens `ApiMappingAgent` → Repo-Analyser **B2** |
| ER diagram from ORM | RepoLens `ErDiagramAgent` → Repo-Analyser **I1** |
| Trace endpoint flow | RepoLens `FlowTraceAgent` → Repo-Analyser **I2** |
| Bug diagnosis (static) | RepoLens `BugDiagnosisAgent` → Repo-Analyser **I6** (add fix + proof) |
| Build FastAPI/Node/Rust from scratch | Repo-Analyser **B4/B5/B6** only (not RepoLens) |
| Docker, Terraform, K8s, CI | Repo-Analyser **I5, D1–D6** only |
| Parallel worktrees | Repo-Analyser **A1/A2** only |

---

## Agent Mapping Table

| Repo-Analyser Task | RepoLens Agent | Overlap |
|-------------|----------------|---------|
...
| D1–D6 Infra | `ArtifactValidationRunner`, etc. | Low — Repo-Analyser infra tasks are platform artifacts |
...
# Transform ApiMapReport → Repo-Analyser B2 api_map_report.json schema
...
These exist in RepoLens but are **not** separate Repo-Analyser eval tasks:
...
- Unified A1–D6 platform pipeline — see old RepoLens registry; Repo-Analyser uses PML/OCL task set instead
...
Repo-Analyser outputs always land in `generated_projects/{run_id}/` regardless of whether RepoLens or Cursor produces them.
